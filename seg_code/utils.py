...
import special_transforms as SegT
import numpy as np
import csv
import torch
...
if __name__ == '__main__':
    ...
    dataset = CityDataset('/mnt/hdd/karmpatel/naman/demo/cityscapes/small_dataset/train', transform=SegT.Compose([SegT.RandomHorizontalFlip(), SegT.Resize((128, 128)), SegT.ToTensor()]))
    dataset0 = CityDataset('/mnt/hdd/karmpatel/naman/demo/cityscapes/small_dataset/train', transform=SegT.Compose([SegT.ToTensor()]))
    dataset1 = CityDataset('/mnt/hdd/karmpatel/naman/demo/cityscapes/small_dataset/train', transform=SegT.Compose([SegT.Resize((128, 128)), SegT.ToTensor()]))
    dataset2 = CityDataset('/mnt/hdd/karmpatel/naman/demo/cityscapes/small_dataset/train', transform=SegT.Compose([SegT.Resize((128, 128)), SegT.RandomHorizontalFlip(), SegT.ToTensor()]))
    dataset3 = CityDataset('/mnt/hdd/karmpatel/naman/demo/cityscapes/small_dataset/train', transform=SegT.Compose([SegT.Resize((128, 128)), SegT.RandomResizedCrop(32), SegT.ToTensor()]))
    dataset4 = CityDataset('/mnt/hdd/karmpatel/naman/demo/cityscapes/small_dataset/train', transform=SegT.Compose([SegT.Resize((128, 128)), SegT.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4), SegT.ToTensor()]))
    dataset_all = CityDataset('/mnt/hdd/karmpatel/naman/demo/cityscapes/small_dataset/train', transform=SegT.Compose([SegT.Resize((128, 128)), SegT.RandomHorizontalFlip(), SegT.RandomResizedCrop(32), SegT.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4), SegT.ToTensor()]))
        
    from pylab import show, imshow, subplot, axis, savefig
...
############### Show the data augmentation methods ###################################################################
    # Loop through the images and plot them with titles
    for i in range(3):
        # Retrieve images and labels from the datasets
        ...
        # Plot the final image with all transformations
        sub5 = subplot(3, 5, 5 * i + 5)
        imshow(F.to_pil_image(im_final))
        if i == 0:
            sub5.set_title("All Transformations")
        axis('off')
